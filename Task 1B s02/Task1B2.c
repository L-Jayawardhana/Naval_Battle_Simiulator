...
int main (){
    ...
    switch (choice){
        case 1 : { 
            FILE *Task_1_B_S2;
            Task_1_B_S2 = fopen("Task 1 B S2.txt", "w"); // Open for writing
            if (Task_1_B_S2 == NULL) {
                printf("Error opening file ..! \n");
                return 1;
            }
            fprintf(Task_1_B_S2, "%35s","Through out the Task 1 B S2\n");
            fprintf(Task_1_B_S2, "%36s","--------------------------\n");
            fprintf(Task_1_B_S2, "\n");
            ...
            fprintf(Task_1_B_S2, "length(km) of one side of the battlefield.(battlefield is a square) : %d * %d square kilometer.\n",Size ,Size);
            fprintf(Task_1_B_S2, "\n");
          ...
          //print the type of battelship, guns and shell velocities as a table to the text file
            ...
            printf("\n");
               ...
               //Asking for battleship coordinates
                ...
                fprintf(Task_1_B_S2,"Choosed %s as your battleship and its velocity is %d m/s.\n" , Batname , FindVelocity1);
                fprintf(Task_1_B_S2,"Your -%s-starting coordinates are (%d , %d)\n" , Batname ,Bx ,By);
                fprintf(Task_1_B_S2, "\n");
            ...
            printf("\n");
                ...
                //Escort ships properties
                    ...
                    fprintf(Task_1_B_S2,"%46s" ,"Types of Escort ships and their species\n");
                    fprintf(Task_1_B_S2,"%-10s %-28s %-27s %-15s %-16s %-15s %-12s %-15s %-15s\n","Notation","Type name","Gun name","Impact power","Max fire range","Angle range","Min Angle","Min Velocity","Max Velocity");
                    fprintf(Task_1_B_S2,"-------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
                    
                    //using for loop to getting inputs from the array and print them
                    for (int i = 0 ; i < 5 ; i++){
                        fprintf(Task_1_B_S2,"%-10s %-28s %-27s %-15.2f %-6.2f %-9s %-15.f %-13.f %-4d %-10s %-4d %-10s\n",
                ...
                printf("\n"); 
                    ...
                    fprintf(Task_1_B_S2,"Escort ships for the battlefield is %d" ,N);//text
                    fprintf(Task_1_B_S2,"\n"); 
            ...
            printf("\n");
                ...
                int maxiteratoin = 100 ;
                for (int k = 0 ; k < maxiteratoin ; k++){
                    ...
                    fprintf(Task_1_B_S2,"Battle details when %s in (%d , %d) position.\n" , Batname , Bx ,By);
                    ...
                    for (int i = 0 ; i < N ; i++){
                        ...
                        int t = Gen_Random (maxiteratoin-10 , 10) ;
                        
                        if (k > t && k < maxiteratoin){
                            float Bmin = INFINITY ;
                            ...
                            if (Distance(Bx , Random_Eschips[i].X_coordinate , By , Random_Eschips[i].Y_coordinate) <= FindMaxFireRange(Batships , B_choice) &&
                                ...
                                fprintf(Task_1_B_S2,"%-25s %-5s %-20s by the %s\n",Random_Eschips[i].Type_name ,"was","not hit", Batname);
                            }
                        }
                        else {
                            //check the max fire range of battleship is less than or more than the distance between escort ships.
                        ...
                        for(int i = 0 ; i < N ; i++){
                            ...
                            //if distance for the batlleship is not between min and max fire range of escort ship,display battleship was not hit by that escort ship
                                ...
                                if(i == N-1){
                                    ...
                                    printf("\n");
                                    fprintf(Task_1_B_S2,"Battleship was not harmed .");//to the text
                                }
                            }
                        }
                    }
                    printf("\n");
                        ...
                        if (Arraysize == N || battleshipHit){
                            if (Arraysize == N){
                                ...
                                fprintf(Task_1_B_S2,"All escort ships was destroyed .\n");
                            } 
                            if (battleshipHit){
                                printf("\x1b[1m"); printf("Battleship was destroyed .\n");printf("\x1b[0m");
                                fprintf(Task_1_B_S2,"Battleship was destroyed .\n");
                            } else {
                                printf("\x1b[1m"); printf("Battleship was not destroyed .\n");printf("\x1b[0m");
                                fprintf(Task_1_B_S2,"Battleship was not destroyed .\n");
                            }
                            printf("\x1b[1m"); printf("Battle is over");printf("\x1b[0m");
                            fprintf(Task_1_B_S2,"Battle is over");
                            break;    
                        }
    ...
    printf("\n");
    fprintf(Task_1_B_S2,"\n");
    fclose(Task_1_B_S2); //close
    printf("Data written to Task 1 B S2.txt successfully.\n");           
       } break;
       case 2:{
