...
int main (){
    ...
    switch (choice){
        case 1 : { 
            FILE *Task_1_C;
            Task_1_C = fopen("Task 1 C.txt", "w"); // Open for writing
            if (Task_1_C == NULL) {
                printf("Error opening file ..! \n");
                return 1;
            }
            fprintf(Task_1_C, "%35s","Through out the Task 1 C\n");
            fprintf(Task_1_C, "%36s","--------------------------\n");
            fprintf(Task_1_C, "\n");
            ...
            fprintf(Task_1_C, "length(km) of one side of the battlefield.(battlefield is a square) : %d * %d square kilometer.\n",Size ,Size);
            fprintf(Task_1_C, "\n");
          ...
          //print the type of battelship, guns and shell velocities as a table to the text file
            ...
            printf("\n");
               ...
               //Asking for battleship coordinates
                ...
                fprintf(Task_1_C,"Choosed %s as your battleship and its velocity is %d m/s.\n" , Batname , FindVelocity1);
                fprintf(Task_1_C,"Your -%s-starting coordinates are (%d , %d)\n" , Batname ,Bx ,By);
                fprintf(Task_1_C, "\n");
            ...
            printf("\n");
                ...
                //Escort ships properties
                    ...
                    fprintf(Task_1_C,"%46s" ,"Types of Escort ships and their species\n");
                    fprintf(Task_1_C,"%-10s %-28s %-27s %-15s %-16s %-15s %-12s %-15s %-15s\n","Notation","Type name","Gun name","Impact power","Max fire range","Angle range","Min Angle","Min Velocity","Max Velocity");
                    fprintf(Task_1_C,"-------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
                    
                    //using for loop to getting inputs from the array and print them
                    for (int i = 0 ; i < 5 ; i++){
                        fprintf(Task_1_C,"%-10s %-28s %-27s %-15.2f %-6.2f %-9s %-15.f %-13.f %-4d %-10s %-4d %-10s\n",
                ...
                printf("\n"); 
                    ...
                    fprintf(Task_1_C,"Escort ships for the battlefield is %d" ,N);//text
                    fprintf(Task_1_C,"\n"); 
            ...
            printf("\n");
                ...
                float impact = 0 ;
                //repitition for (maxiteratoin) iterations
                ...
                for (int k = 0 ; k < maxiteration ; k++){
                    ...
                    fprintf(Task_1_C,"Battle details when %s in (%d , %d) position.\n" , Batname , Bx ,By);
                    ...
                    for (int i = 0 ; i < N ; i++){
                        ...
                        //assume gun get jammed after t iteration
                        //generate random t between 0 to max k
                        int t = Gen_Random (maxiteration-10 , 10) ;

                        if (k > t && k < maxiteration){
                            float Bmin = INFINITY ;
                            int A = Gen_Random(30 , 0); //after t iteration vertical angle is (0 < theta min < 30) - 90
                            for (int i = A ; i < 90 ; i++){
                                // find the min distance , battleship can fire
                                if (Find_Max_Min_Distance(Random_Eschips[i].Max_velocity , i) < min){
                                    Bmin = Find_Max_Min_Distance(Random_Eschips[i].Max_velocity , i);
                                } 
                            }
                            //check the max fire range of battleship is less than or more than the distance between escort ships
                            //but more realstic i assume max distance is that ship actual fire distance accoring to true data 
                            if (Distance(Bx , Random_Eschips[i].X_coordinate , By , Random_Eschips[i].Y_coordinate) <= FindMaxFireRange(Batships , B_choice) &&
                                Distance(Bx , Random_Eschips[i].X_coordinate , By , Random_Eschips[i].Y_coordinate) >=  Bmin){
                    ...
                    //if battle ship not destroyed by the escorts in initial position, then battleship is safe
                    if (k == 0){
                    ...
                    printf("%30s\n" , "Battleship details");
                        ...
                        for(int l = 0 ; l < N ; l++){
                            ...
                            for (int j = Random_Eschips[l].Min_angle ; j <= (Random_Eschips[l].Min_angle + Random_Eschips[l].Angle_range) ; j++){
                                ...
                                if (Find_Max_Min_Distance(Random_Eschips[l].Max_velocity , j) > max ) {
                                    max = Find_Max_Min_Distance(Random_Eschips[l].Max_velocity , j);
                                } 
                                //getting the min fire range by using min velocity of the shell and angle
                                if (Find_Max_Min_Distance(Random_Eschips[l].Min_velocity , j) < min){
                                    min = Find_Max_Min_Distance(Random_Eschips[l].Min_velocity , j) ;
                                }
                            }
                            //check max distance of shell can go is less or more than max fire range
                            //max fire ranges are acoording to true data 
                            if (max > Random_Eschips[l].Max_Fire_Range){
                                max = Random_Eschips[l].Max_Fire_Range ;
                            }
                            //if min fire range is more than max fire range we assume min = max fire range - 8(km) ; 
                            if (min > Random_Eschips[l].Max_Fire_Range){
                                min = Random_Eschips[l].Max_Fire_Range - 8 ;
                            }
                            ...
                            if (Distance(Bx , Random_Eschips[l].X_coordinate , By , Random_Eschips[l].Y_coordinate) <= max && 
                                Distance(Bx , Random_Eschips[l].X_coordinate , By , Random_Eschips[l].Y_coordinate) >= min){

                                impact = impact + Random_Eschips[l].Impact_power ; //add impact power of attacked escort to the impact of battleship

                                printf("%-25s was hit by the %-25s on %02d:%02d:%02d  .total impact = %.f %%\n",Batname  ,Random_Eschips[l].Type_name,localTime->tm_hour, localTime->tm_min, localTime->tm_sec, impact*100);
                                fprintf(Task_1_C,"%-25s was hit by the %-25s.total impact = %.f %%\n",Batname  ,Random_Eschips[l].Type_name, impact*100); 
                                                                
                                if (impact >= 1){
                                    battleshipHit = true; //change the value
                                    printf("100 %% impact on the Battleship\n" );
                                    fprintf(Task_1_C,"100 %% impact on the Battleship\n" );
                                    break ; 
                                }         
                            } 
                            //if distance for the batlleship is not between min and max fire range of escort ship,display battleship was not hit by that escort ship
                                ...
                                // after the battle there is not nay impact on the battleship, print not a scratch
                                if(l == N-1){
                                    if (impact == 0){
                                        printf("\x1b[1m") ; printf("Not a scratch on Battleship.\n");printf("\x1b[0m") ;
                                        fprintf(Task_1_C,"Not a scratch on Battleship.\n");
                                    }
                                }
                            }
                        }
                        ...
                        if (Arraysize == N || battleshipHit){
                                printf("\x1b[1m"); printf("\x1b[31m");
                            printf("Summary\n");
                                printf("\x1b[0m");
                            fprintf(Task_1_C,"Summary\n");

                            if (Arraysize == N){
                                printf("\x1b[1m");printf("All escort ships was destroyed !\n"); printf("\x1b[0m");
                                fprintf(Task_1_C,"All escort ships was destroyed !\n");
                            } 
                            if (battleshipHit){
                                printf("\x1b[1m"); printf("Battleship was destroyed !\n");printf("\x1b[0m");
                                fprintf(Task_1_C,"Battleship was destroyed !\n");
                            } else {
                                    printf("\x1b[1m"); 
                                printf("imapct on the Battleship is %.f %%\n" , impact*100); // impact display as a precentage
                                printf("Battleship is Alive !\n");
                                    printf("\x1b[0m");

                                fprintf(Task_1_C,"imapct on the Battleship is %.f %%\n" , impact*100); // impact display as a precentage
                                fprintf(Task_1_C,"Battleship is Alive !\n");
                            }
                            printf("\x1b[1m"); printf("Battle is over !\n");printf("\x1b[0m");
                            printf("\n");

                            fprintf(Task_1_C,"Battle is over !\n");
                            fprintf(Task_1_C,"\n");
                    ...
                    if (k == maxiteration-1){
                        printf("There is non destroyed battleship and escort ships here .\n");
                        fprintf(Task_1_C ,"There is non destroyed battleship and escort ships here .\n");
                    }
    ...
    printf("\n");
    fprintf(Task_1_C ,"\n");
    fclose(Task_1_C); //close
    printf("Data written to Task 1 C.txt successfully.\n");       
        break;   
       }   
      case 2:
